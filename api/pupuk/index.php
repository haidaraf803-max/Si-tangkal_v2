...
try {
    $model = new PupukModel($pdo);

    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        apiRequirePermission($pdo, 'pemakaian_pupuk', 'view');
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        apiRequirePermission($pdo, 'pemakaian_pupuk', 'create');
        $body = json_decode(file_get_contents('php://input'), true) ?: $_POST;
        $userId = (int) ($_SESSION['admin']['UserId'] ?? 0);

        if (empty($body['tanggal']) || empty($body['jenis_pupuk']) || empty($body['lokasi'])) {
            http_response_code(422);
            echo json_encode(['success' => false, 'message' => 'tanggal, jenis_pupuk, dan lokasi wajib diisi']);
            exit;
        }
