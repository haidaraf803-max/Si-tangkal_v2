...
class Auth
{
    public static function attempt(string $username, string $password): bool
    {
        try {
            ...
            // Alias supaya endpoint lama (mis. api/monitoring/*.php) yang
            // membaca $_SESSION['user_id'] langsung tetap jalan, tidak
            // peduli login lewat /login.php (web) atau api/auth/login.php
            // (konsumen API) — SATU login untuk SEMUA jalur.
            $_SESSION['user_id']  = $user['UserId'] ?? null;
            $_SESSION['nama']     = $user['Name'] ?? $username;
    ...
    {
        unset($_SESSION['admin'], $_SESSION['sitangkal_user'], $_SESSION['user_id'], $_SESSION['nama'], $_SESSION['username']);
        session_unset();
        session_destroy();
    }
